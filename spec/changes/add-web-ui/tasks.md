...
- [x] 5.3 构建产物接入 server 静态托管，浏览器端到端手工冒烟（连接测试容器→执行样例任务→观察日志/进度）## 验证修复（原地打回第 2 轮）

- [ ] 先复核失败有效性：逐条核对 `spec/changes/add-web-ui/verification-report.md` 中失败场景的期望断言（状态字面量/选择器/前置数据）与产品实际行为是否相符——断言与产品不符属场景缺陷，修场景文件；确属产品行为不符才修产品代码
